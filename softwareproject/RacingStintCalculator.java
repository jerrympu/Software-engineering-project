...
public class RacingStintCalculator {
    ...
    private final Duration raceParam; // Duration for Time Trial, Laps count for Circuit Race
    ...
    public Duration calculateTotalTime() {
        return raceMode == RaceMode.TIME_TRIAL 
    ...
    private Duration calculateTimeTrialDuration() {
        ...
        double fuelUsed = totalLaps * (fuelCapacity / 10); // 10 laps per tank assumption
    ...
    private Duration calculateCircuitRaceDuration() {
        long totalLaps = raceParam.toMinutes(); // Storing lap count in minutes
    ...
    private static void displayResults(RacingStintCalculator calculator) {
        try {
            ...
            // Additional metrics
            double pitStops = calculator.raceMode == RaceMode.TIME_TRIAL
                ? calculator.raceParam.toMinutes() / 45.0  // Time Trial estimation
